...
def compute_map_and_print(dataset, featuretype, mode, ranks, gnd, kappas=[1, 5, 10], verbose=False):
    ...
    # new evaluation protocol
        ...
        print('>> Test Dataset: {} *** {} >>'.format(dataset, featuretype))
        print('>> {}: mAP Eeay: {}, Medium: {}, Hard: {}'.format(mode, np.around(mapE * 100, decimals=2), np.around(mapM * 100, decimals=2), np.around(mapH * 100, decimals=2)))
        print('>> {}: mP@k{} Easy: {}, Medium: {}, Hard: {}'.format(mode, kappas, np.around(mprE * 100, decimals=2), np.around(mprM * 100, decimals=2), np.around(mprH * 100, decimals=2)))
